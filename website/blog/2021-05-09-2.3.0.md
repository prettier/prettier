...
And just a reminder, when Prettier is installed or updated, it’s [strongly recommended](https://prettier.io/docs/install#summary) to specify the exact version in `package.json`: `"2.3.0"`, not `"^2.3.0"`.
...
Files with the extensions `.hbs` and `.handlebars` are recognized as Handlebars by default. For other extensions, the [`--parser`](/docs/configuration#setting-the-parser-option) option with the value `glimmer` has to be specified – for example, using command line or, better yet, [configuration overrides].
...
[parser-option]: https://prettier.io/docs/options#parser
[configuration overrides]: https://prettier.io/docs/configuration#configuration-overrides
...
Previously, when [range formatting](https://prettier.io/docs/options#range) was performed, such nodes were considered part of the range, now they're excluded. This affects other languages that the range formatting feature supports, not only JavaScript.
...
See also an example in the [docs](https://prettier.io/docs/plugins#print).
