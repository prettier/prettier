...
This release adds HTML, Vue, Angular and MDX support,
respecting decorator position, and parser inference via shebang.
It also adds support for several new syntax and has a few formatting tweaks.
...
#### Flatten else-branch for nested ternaries ([#5039] by [@suchipi], [#5272] by [@duailibe])

Previously, nested ternaries are always indented, which causes _infinite stairs_ for deep-nested ternaries.
To solve this problem, we flattened the else-branch for nested ternaries in an `if..elseif..else` fashion.
...
In Prettier 1.14, we tried to solve an issue that decorators were always inlined,
we thought inline decorators were rare cases so we could put them on their own line,
but unfortunately it's not the case.
Then we tried to find another better heuristic to identify when should we inline them,
but we came up with the result after a long discussion in [#4924]
that there's no a reliable to identify them,
so we decided to respect the original source in Prettier 1.15.
...
Decorators are still not standardized and
[where should decorators on exported classes go] is not yet decided.
To help proposal authors to get some feedbacks,
Babel 7 added support for both decorators before and after the exported classes.
Prettier 1.15 adds support for them and respects where did you put decorator.
...
[where should decorators on exported classes go]: https://babeljs.io/blog/2018/09/17/decorators#where-should-decorators-on-exported-classes-go
...
Flow team [planned to treat all object type _exact_ by default][inexact] in the future,
so they introduced a new syntax to indicate if an object type is inexact.
This syntax is now supported with `--parser flow` in Prettier 1.15.
...
Previously, parentheses that surrounds Flow typecast comments are sometimes removed,
which no longer satisfies Flow's syntax. This issue has been fixed in Prettier 1.15.
...
Previously, math syntaxes are sometimes mangled since we treat them as normal text.
They are now preserved in Prettier 1.15 so can use safely use math syntax thanks to [remark-math].
...
Previously, we use filename and extension to infer which parser to use,
but often there's no extension for CLI scripts,
which is not ideal since we have to specify parser manually.
In Prettier 1.15, ,
we'll look into the first line of the file if there's no extension,
and use it to infer which parser to use if there's a shebang.
...
Previously, there's no way to trim whitespaces in the current line,
but we can use some workarounds to achieve the same goal.
Unfortunately, when the situation becomes more complex (e.g., preprocessor directives in C#),
workarounds are not available anymore,
so we introduce a new [trim] command to trim whitespaces in a reliable way.
...
Previously, it was done in the parser but this way it also exposed the internal stuff to external users (custom parser), which is not ideal.
In Prettier 1.15, you can now use `printer.preprocess` to preprocess the AST without exposing any internal stuff.
...
Previously, loading a config file that its format is not unsupported will throw an error message
that looks like an unexpected error, we improved the message in Prettier 1.15.
...
Previously, parentheses for mixed exponentiation/modulo are wrongly removed,
this is fixed in Prettier 1.15.
...
In the previous version, some comments in `try..finally` are printed in a wrong order,
Prettier now prints it correctly.
...
Comments in catch clause are now printed on their own line just like other clauses.
...
an argument that's an arrow function with conditional expression as body.
We inlined them in Prettier 1.15.
...
In previous version, comments in variable declaration caused variable declarator to be printed without indentation,
this has been fixed in Prettier 1.15.
...
Necessary parens for ternary in optional member expression are printed correctly in Prettier 1.15.
...
Previously, interpolation-like strings are wrongly unescaped in embedded GraphQL,
which led JavaScript treated it as interpolation. They're correctly escaped in Prettier 1.15.
...
Previously, Prettier strips quotes in keys if they're not necessary in es6,
which causes the output not compatible with es5.
...
#### Do not hug if the second argument in function is a ternary ([#5151] by [@onurtemizkan])

We have some special cases that we'll _hug_ arguments
if the first one is a function and the second one is a not a complex expression,
but apparently ternary is not the case
so we are not going to hug it in this case in Prettier 1.15.
...
Previously, arguments in `beforeEach` are wrongly hugged, we've fixed this issue in Prettier 1.15.
...
#### Preserve dangle comments in new expression ([#5017] by [@flxwu])

Dangling comments in new expression are preserved in Prettier 1.15.
...
Logical expressions in ternary in function call are now indented correctly.
...
Comments in `while` are now correctly attached
so that it does not need to be formatted twice to get the stable output.
...
Comments between function declaration and its body does not need to be formatted twice to get the final result now.
...
Unnecessary indentation for logical expression chains are now disabled.
...
#### Do not change non-breaking whitespace into the regular one ([#5165] by [@vjeux])

Previously, non-breaking whitespaces are treated as regular whitespaces,
which casued them to be replaced with regular whitespaces.
This issue has been fixed in Prettier 1.15.
...
Simple jsx opening tags are now force unbroken.
...
Generics with single identifer are now always inlined.
...
Unnecessary parens in `export default async` are now removed.
...
Necessary semicolons for non-null assertion won't be removed in `--no-semi` mode.
...
that there're modifiers in its next property, which does not need to do ASI protection.
...
Previously, parens for complex nodes in class expression's `extends` are wrongly removed and produces invalid code.
This issue has been fixed in Prettier 1.15.
...
Previously, necessary parens for optional types in tuple are wrongly removed and produces invalid code.
This issue has been fixed in Prettier 1.15.
...
In Prettier 1.14, the AST location informations were wrongly shifted
due to how we extract front matters from the source,
which led `/* prettier-ignore */` produces invalid output.
...
Previously, we only detected front matters that there's no trailing spaces for their delimiters,
which led an issue that thematic breaks (i.e., `---`) are wrongly recognized as the end of the front matter.
...
Previously, we always insert whitespaces between Latin and CJK characters to improve the readability,
but according to the feedback we received, Korean uses conventional spaces,
inserting whitespaces would cause issues for them
so we disabled this behavior for Hangul in Prettier 1.15.
...
which causes other plugins (e.g., php) failed to format these code blocks.
...
Previously, anything that does not fit print width in footnote definition will be break into multiple lines,
but breaking a single line paragraph did not improve anything except verbosity so we inlined them in Prettier 1.15.
...
which we need to format it twice to get it stable but it also produces the wrong output.
This issue has been fixed in Prettier 1.15.
...
Previously, string with escaped quotes in doubleQuote will cause invalid output,
...
In Prettier 1.14, trailing comments for document and document head may be moved,
they're now always preserved as their trailing comments.
...
which produce syntax errors since implicit keys with more than [1024 characters] are not allowed.
This issue has been fixed in Prettier 1.15.
...
Previously, an mapping item with empty mapping value is always printed as [explicit key],
which is not that common and people are confused.
In Prettier 1.15, we always print it as implicit key if appropriate.
