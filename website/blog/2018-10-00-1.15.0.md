...
We also add support for the `html` template tag (or a “tag” comment containing `HTML`):
...
Previously, nested ternaries were always indented, which causing increasing levels of indentation for deeply-nested ternaries.
...
We thought inline decorators were rare cases so we could always place decorators on their own line,
...
so we decided to respect the original formatting style in Prettier 1.15.
...
Prettier 1.15 adds a boolean option `--jsx-single-quote` (or `jsxSingleQuote` in the config)
for printing single quotes in JSX.
...
but we can use some workarounds to achieve the same goal. There wasn’t a reliable workaround,
...
Necessary parens for ternary in optional member expression (`?.`) are now printed correctly in Prettier 1.15.
...
Previously, Prettier stripped quotes in keys if they weren’t necessary in ES6,
which causes the output to be incompatible with ES5.
...
We have some special cases that we'll _hug_ function call arguments
if the first one is a function and the second one is a not a complex expression,
but we counted ternaries as complex expressions. They can actually be complex,
...
Passing a comment instead of an expression to a `new` expression is now preserved
...
but breaking a single line paragraph did was not much of a readability improvement so we always inline them in Prettier 1.15.
...
which is not that common and people were confused.
...
as well as those who raises issues and gives us feedbacks.
Prettier is a community-drive project
and is able to continue to exist thanks to people like you. Thank you!

Special thanks [@j-f1] for reviewing this blog post!
