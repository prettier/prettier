...
This happens because whitespaces are significant in inline elements.
...
- follows the default CSS `display` value for every element to identify if the whitespace is significant,
- and borrows the opening or closing tags (if necessary) to avoid adding or removing significant whitespaces.
...
- `strict` - All whitespaces are considered significant.
- `ignore` - All whitespaces are considered insignificant.
