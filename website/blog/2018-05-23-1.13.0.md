...
#### Support jest-each template string ([#4423] by [@ikatyang])

`jest@beta` introduces a new feature called [data driven tests](https://github.com/facebook/jest/pull/6102) where you can describe examples to test in a table. Prettier 1.13 includes support for this feature, and will automatically indent your examples table when using data driven tests.
...
#### Split JSON and JSON5 ([#4367] by [@ikatyang], [#4371] by [@ikatyang], [#4333] by [@duailibe])
...
#### Fix: lazy match the frontmatter in postcss parser ([#4457] by [@eliasmeire])

#### fix: css variables and scss interpolation ([#4471] by [@evilebottnawi])

#### Fix: Always preserve frontmatter, remove dependency ([#4392] by [@eliasmeire])
...
#### feat(typescript): support TSImportType ([#4429] and [#4438] by [@ikatyang])
...
#### feat(typescript): support TaggedTemplateExpression typeParameters ([#4353] by [@ikatyang])
...
#### feat(typescript): support generic JSX element ([#4268] by [@ikatyang])
