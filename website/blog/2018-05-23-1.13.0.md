...
When installing Prettier and plugins globally, Prettier was not able to automatically load them because it searched for them in the nearest `package.json`. Now Prettier will look for plugins in the same `node_modules` it's installed. Additionally, for cases where Prettier can't find the plugins automatically (because of different setups than the one mentioned above), a new `--plugin-search-dir` option was added so you can specify where Prettier should search for plugins.
...
Prettier 1.13 now vastly improves the cursor offset tracking for several cases where Prettier was outputing the wrong location, making editor integrations move the cursor to the wrong location.
...
When passing `--prettier=path/to/plugin` Prettier 1.12 would crash because it looked for a `path/to/plugin` in `node_modules` folder. Prettier 1.13 will look in the current directory first and failing that will look in `node_modules`.
...
Prettier will now add parens around module operations when they're inside an additive operation, even though they're not necessary. This is too help more quickly understand a code snippet.
...
When mixing two different multiplicative operations, Prettier will add parens to make it more clear how the computer will execute it.
...
Prettier 1.12 accidentally changed the meaning of types when using nullable operator because of the precedence of operators. This is now fixed in Prettier 1.13.
...
#### Allow trailing commas in SCSS list and maps ([#4317] by [@ad1992])

Prettier now prints trailing commas in lists and maps in SCSS if `trailingComma` is not `"none"` and it's broken in multiple lines.
