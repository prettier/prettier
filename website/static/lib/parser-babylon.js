	...
	var parserBabylon_1 = createCommonjsModule(function (module) {
		...
		function createError$1(t, e) {
			...
			function s(t) {
				...
				}), functionExpression: new V("function", !0) };C.parenR.updateContext = C.braceR.updateContext = function () {
				...
				var e = t === C._if || t === C._for || t === C._with || t === C._while;this.state.context.push(e ? U.parenStatement : U.parenExpression), this.state.exprAllowed = !0;
				...
				function e(e, s) {
					...
					var e = 42 === t ? C.star : C.modulo,
					    s = 1,
					    i = this.input.charCodeAt(this.state.pos + 1);return 42 === t && 42 === i && (s++, i = this.input.charCodeAt(this.state.pos + 2), e = C.exponent), 61 === i && (s++, e = C.assign), this.finishOp(e, s);
					...
					var e = this.input.charCodeAt(this.state.pos + 1);return 61 === e ? this.finishOp(C.equality, 61 === this.input.charCodeAt(this.state.pos + 2) ? 3 : 2) : 61 === t && 62 === e ? (this.state.pos += 2, this.finishToken(C.arrow)) : this.finishOp(61 === t ? C.eq : C.bang, 1);
					...
					switch (t) {case 35:
							if (this.hasPlugin("classPrivateProperties") && this.state.classLevel > 0) return ++this.state.pos, this.finishToken(C.hash);this.raise(this.state.pos, "Unexpected character '" + c(t) + "'");case 46:
							...
							return this.finishOp(C.tilde, 1);}this.raise(this.state.pos, "Unexpected character '" + c(t) + "'");
					...
					}return t === C._return ? F.test(this.input.slice(this.state.lastTokEnd, this.state.start)) : t === C._else || t === C.semi || t === C.eof || t === C.parenR || (t === C.braceL ? this.curContext() === U.braceStatement : t === C.relational || !this.state.exprAllowed);
				...
				function e() {
					...
					return e.sourceType = this.options.sourceType, this.parseBlockBody(e, !0, !0, C.eof), t.program = this.finishNode(e, "Program"), t.comments = this.state.comments, this.options.tokens && (t.tokens = this.state.tokens), this.finishNode(t, "File");
					...
					var s = this.state.type,
					    i = this.startNode();switch (s) {case C._break:case C._continue:
							...
							if (this.lookahead().type === C.dot) break;return t || this.unexpected(), this.parseFunctionStatement(i);case C._class:
					...
					this.expectOnePlugin(["decorators", "decorators2"]);var t = this.startNode();if (this.next(), this.hasPlugin("decorators2")) {
						for (var e = this.state.start, s = this.state.startLoc, i = this.parseIdentifier(!1); this.eat(C.dot);) {
							...
							var a = this.startNodeAt(e, s);a.callee = i, this.state.decoratorStack.push([]), a.arguments = this.parseCallExpressionArguments(C.parenR, !1), this.state.decoratorStack.pop(), i = this.finishNode(a, "CallExpression"), this.toReferencedList(i.arguments);
						}t.expression = i;
					...
					this.next(), t.discriminant = this.parseParenExpression();var e = t.cases = [];this.expect(C.braceL), this.state.labels.push(tt);for (var s, i = void 0; !this.match(C.braceR);) {
					...
					var i = t.declarations = [];for (t.kind = s.keyword;;) {
						var r = this.startNode();if (this.parseVarHead(r), this.eat(C.eq) ? r.init = this.parseMaybeAssign(e) : (s !== C._const || this.match(C._in) || this.isContextual("of") ? "Identifier" === r.id.type || e && (this.match(C._in) || this.isContextual("of")) || this.raise(this.state.lastTokEnd, "Complex binding patterns require an initialization value") : this.hasPlugin("typescript") || this.unexpected(), r.init = null), i.push(this.finishNode(r, "VariableDeclarator")), !this.eat(C.comma)) break;
					}return t;
					...
					if (this.shouldParseExportStar()) {
						...
						var e = this.startNode();e.exported = this.parseIdentifier(!0);var s = [this.finishNode(e, "ExportDefaultSpecifier")];if (t.specifiers = s, this.match(C.comma) && this.lookahead().type === C.star) {
							this.expect(C.comma);var i = this.startNode();this.expect(C.star), this.expectContextual("as"), i.exported = this.parseIdentifier(), s.push(this.finishNode(i, "ExportNamespaceSpecifier"));
						...
						if (this.eat(C._default)) {
							...
							if (this.isContextual("async")) {
								var n = this.lookahead();n.type !== C._function && this.unexpected(n.start, "Unexpected token, expected function");
					...
					return this.match(C.string) ? (t.specifiers = [], t.source = this.parseExprAtom()) : (t.specifiers = [], this.parseImportSpecifiers(t), this.expectContextual("from"), t.source = this.match(C.string) ? this.parseExprAtom() : this.unexpected()), this.semicolon(), this.finishNode(t, "ImportDeclaration");
				...
				function e() {
					...
					this.nextToken();var t = this.parseExpression();return this.match(C.eof) || this.unexpected(), t.comments = this.state.comments, t;
					...
					var e = this.state.potentialArrowAt === this.state.start,
					    s = void 0;switch (this.state.type) {case C._super:
							...
							return this.parseBooleanLiteral();case C.parenL:
					...
					}return t.callee = this.parseNoCallExpr(), this.eat(C.questionDot) && (t.optional = !0), this.parseNewArguments(t), this.finishNode(t, "NewExpression");
				}, e.prototype.parseNewArguments = function (t) {
					if (this.eat(C.parenL)) {
						var e = this.parseExprList(C.parenR);this.toReferencedList(e), t.arguments = e;
					} else t.arguments = [];
					...
					var s = [],
						...
						if (r) r = !1;else if (this.expect(C.comma), this.eat(C.braceR)) break;if (this.match(C.at)) if (this.hasPlugin("decorators2")) this.raise(this.state.start, "Stage 2 decorators disallow object literal property decorators");else for (; this.match(C.at);) {
					...
					return s || e || this.match(C.parenL) ? (i && this.unexpected(), t.kind = "method", t.method = !0, this.parseMethod(t, e, s, !1, "ObjectMethod")) : this.isGetterOrSetterMethod(t, i) ? ((e || s) && this.unexpected(), t.kind = t.key.name, this.parsePropertyName(t), this.parseMethod(t, !1, !1, !1, "ObjectMethod"), this.checkGetterSetterParamCount(t), t) : void 0;
					...
					var a = this.state.inMethod;this.state.inMethod = t.kind || !0, this.initFunction(t, s), this.expect(C.parenL);var n = i;return t.params = this.parseBindingList(C.parenR, !1, n), t.generator = !!e, this.parseFunctionBodyAndFinish(t, r), this.state.inMethod = a, t;
					...
					e || this.checkReservedWord(this.state.value, this.state.start, !!this.state.type.keyword, !1);var s = void 0;if (this.match(C.name)) s = this.state.value;else {
				...
				function e() {
					...
					for (var i = [], r = !0; !this.eat(t);) {
						if (r ? r = !1 : this.expect(C.comma), e && this.match(C.comma)) i.push(null);else {
							if (this.eat(t)) break;if (this.match(C.ellipsis)) {
								i.push(this.parseAssignableListItemTypes(this.parseRest())), this.expect(t);break;
							}var a = [];for (this.match(C.at) && this.hasPlugin("decorators2") && this.raise(this.state.start, "Stage 2 decorators cannot be used to decorate parameters"); this.match(C.at);) {
				...
				return function (t) {
					function e() {
						...
						return (!this.match(C.name) || "type" !== this.state.value && "interface" !== this.state.value && "opaque" != this.state.value) && t.prototype.isExportDefaultSpecifier.call(this);
						...
						return t.prototype.shouldParseExportStar.call(this) || this.isContextual("type") && this.lookahead().type === C.star;
						...
						}return this.match(C.colon) && (t.typeAnnotation = this.flowParseTypeAnnotation()), this.finishNode(t, t.type), t;
			    ...
			    ht = /^[\da-fA-F]+$/,
			    pt = /^\d+$/;U.j_oTag = new V("<tag", !1), U.j_cTag = new V("</tag", !1), U.j_expr = new V("<tag>...</tag>", !0, !0), C.jsxName = new k("jsxName"), C.jsxText = new k("jsxText", { beforeExpr: !0 }), C.jsxTagStart = new k("jsxTagStart", { startsExpr: !0 }), C.jsxTagEnd = new k("jsxTagEnd"), C.jsxTagStart.updateContext = function () {
				...
				var e = this.state.context.pop();e === U.j_oTag && t === C.slash || e === U.j_cTag ? (this.state.context.pop(), this.state.exprAllowed = this.curContext() === U.j_expr) : this.state.exprAllowed = !0;
				...
				return function (t) {
					function e() {
						...
						if (this.state.inPropertyName) return t.prototype.readToken.call(this, e);var s = this.curContext();if (s === U.j_expr) return this.jsxReadToken();if (s === U.j_oTag || s === U.j_cTag) {
							if (a(e)) return this.jsxReadWord();if (62 === e) return ++this.state.pos, this.finishToken(C.jsxTagEnd);if ((34 === e || 39 === e) && s === U.j_oTag) return this.jsxReadString(e);
						...
						if (this.match(C.braceL)) {
							...
							if (!this.match(C.slash) || e !== C.jsxTagStart) return t.prototype.updateContext.call(this, e);this.state.context.length -= 2, this.state.context.push(U.j_cTag), this.state.exprAllowed = !1;
						}
					}, e;
				}(t);
			},
				...
				return function (t) {
					function e() {
						...
						return this.next(), !(this.hasPrecedingLineBreak() || this.match(C.parenL) || this.match(C.colon) || this.match(C.eq) || this.match(C.question));
						...
						switch (t) {case "EnumMembers":case "TypeMembers":
								return this.match(C.braceR);case "HeritageClauseElement":
								return this.match(C.braceL);case "TupleElementTypes":
								return this.match(C.bracketR);case "TypeParametersOrArguments":
						...
						for (var i = [];;) {
							if (this.tsIsListTerminator(t)) break;var r = e();if (null == r) return;if (i.push(r), !this.eat(C.comma)) {
								if (this.tsIsListTerminator(t)) break;return void (s && this.expect(C.comma));
							}
						}return i;
					}, e.prototype.tsParseBracketedList = function (t, e, s, i) {
						i || (s ? this.expect(C.bracketL) : this.expectRelational("<"));var r = this.tsParseDelimitedList(t, e);return s ? this.expect(C.bracketR) : this.expectRelational(">"), r;
						...
						var t = this.startNode();return this.expect(C._typeof), t.exprName = this.tsParseEntityName(!0), this.finishNode(t, "TSTypeQuery");
						...
						var t = this;return this.parseBindingList(C.parenR).map(function (e) {
						...
						var e = this.startNode();return "TSConstructSignatureDeclaration" === t && this.expect(C._new), this.tsFillSignature(C.colon, e), this.tsParseTypeMemberSemicolon(), this.finishNode(e, t);
						...
						if (this.match(C.bracketL) && this.tsLookAhead(this.tsIsUnambiguouslyIndexSignature.bind(this))) {
							this.expect(C.bracketL);var e = this.parseIdentifier();this.expect(C.colon), e.typeAnnotation = this.tsParseTypeAnnotation(!1), this.expect(C.bracketR), t.parameters = [e];var s = this.tsTryParseTypeAnnotation();return s && (t.typeAnnotation = s), this.tsParseTypeMemberSemicolon(), this.finishNode(t, "TSIndexSignature");
						}
					}, e.prototype.tsParsePropertyOrMethodSignature = function (t, e) {
						this.parsePropertyName(t), this.eat(C.question) && (t.optional = !0);var s = t;if (e || !this.match(C.parenL) && !this.isRelational("<")) {
						...
						}var a = s;return this.tsFillSignature(C.colon, a), this.tsParseTypeMemberSemicolon(), this.finishNode(a, "TSMethodSignature");
					}, e.prototype.tsParseTypeMember = function () {
						if (this.match(C.parenL) || this.isRelational("<")) return this.tsParseSignatureMember("TSCallSignatureDeclaration");if (this.match(C._new) && this.tsLookAhead(this.tsIsStartOfConstructSignature.bind(this))) return this.tsParseSignatureMember("TSConstructSignatureDeclaration");var t = this.startNode(),
						...
						return this.next(), this.match(C.parenL) || this.isRelational("<");
						...
						this.expect(C.braceL);var t = this.tsParseList("TypeMembers", this.tsParseTypeMember.bind(this));return this.expect(C.braceR), t;
					}, e.prototype.tsIsStartOfMappedType = function () {
						return this.next(), this.isContextual("readonly") && this.next(), !!this.match(C.bracketL) && (this.next(), !!this.tsIsIdentifier() && (this.next(), this.match(C._in)));
					}, e.prototype.tsParseMappedTypeParameter = function () {
						var t = this.startNode();return t.name = this.parseIdentifierName(t.start), this.expect(C._in), t.constraint = this.tsParseType(), this.finishNode(t, "TypeParameter");
					}, e.prototype.tsParseMappedType = function () {
						var t = this.startNode();return this.expect(C.braceL), this.eatContextual("readonly") && (t.readonly = !0), this.expect(C.bracketL), t.typeParameter = this.tsParseMappedTypeParameter(), this.expect(C.bracketR), this.eat(C.question) && (t.optional = !0), t.typeAnnotation = this.tsTryParseType(), this.semicolon(), this.expect(C.braceR), this.finishNode(t, "TSMappedType");
						...
						var t = this.startNode();return this.expect(C.parenL), t.typeAnnotation = this.tsParseType(), this.expect(C.parenR), this.finishNode(t, "TSParenthesizedType");
					}, e.prototype.tsParseFunctionOrConstructorType = function (t) {
						var e = this.startNode();return "TSConstructorType" === t && this.expect(C._new), this.tsFillSignature(C.arrow, e), this.finishNode(e, t);
					}, e.prototype.tsParseLiteralTypeNode = function () {
						var t = this,
						    e = this.startNode();return e.literal = function () {
							switch (t.state.type) {case C.num:
									return t.parseLiteral(t.state.value, "NumericLiteral");case C.string:
									return t.parseLiteral(t.state.value, "StringLiteral");case C._true:case C._false:
									return t.parseBooleanLiteral();default:
									throw t.unexpected();}
						}(), this.finishNode(e, "TSLiteralType");
					}, e.prototype.tsParseNonArrayType = function () {
						switch (this.state.type) {case C.name:case C._void:case C._null:
								var t = this.match(C._void) ? "TSVoidKeyword" : this.match(C._null) ? "TSNullKeyword" : x(this.state.value);if (void 0 !== t && this.lookahead().type !== C.dot) {
									var e = this.startNode();return this.next(), this.finishNode(e, t);
								}return this.tsParseTypeReference();case C.string:case C.num:case C._true:case C._false:
								return this.tsParseLiteralTypeNode();case C.plusMin:
								if ("-" === this.state.value) {
									var s = this.startNode();if (this.next(), !this.match(C.num)) throw this.unexpected();return s.literal = this.parseLiteral(-this.state.value, "NumericLiteral", s.start, s.loc.start), this.finishNode(s, "TSLiteralType");
								}break;case C._this:
								var i = this.tsParseThisTypeNode();return this.isContextual("is") && !this.hasPrecedingLineBreak() ? this.tsParseThisTypePredicate(i) : i;case C._typeof:
								return this.tsParseTypeQuery();case C.braceL:
								return this.tsLookAhead(this.tsIsStartOfMappedType.bind(this)) ? this.tsParseMappedType() : this.tsParseTypeLiteral();case C.bracketL:
								return this.tsParseTupleType();case C.parenL:
								return this.tsParseParenthesizedType();}throw this.unexpected();
					}, e.prototype.tsParseArrayTypeOrHigher = function () {
						for (var t = this.tsParseNonArrayType(); !this.hasPrecedingLineBreak() && this.eat(C.bracketL);) {
							if (this.match(C.bracketR)) {
								var e = this.startNodeAtNode(t);e.elementType = t, this.expect(C.bracketR), t = this.finishNode(e, "TSArrayType");
							} else {
								var s = this.startNodeAtNode(t);s.objectType = t, s.indexType = this.tsParseType(), this.expect(C.bracketR), t = this.finishNode(s, "TSIndexedAccessType");
							}
						}return t;
						...
						return !(!this.match(C.name) && !this.match(C._this)) && (this.next(), !0);
					}, e.prototype.tsIsUnambiguouslyStartOfFunctionType = function () {
						if (this.next(), this.match(C.parenR) || this.match(C.ellipsis)) return !0;if (this.tsSkipParameterStart()) {
							if (this.match(C.colon) || this.match(C.comma) || this.match(C.question) || this.match(C.eq)) return !0;if (this.match(C.parenR) && (this.next(), this.match(C.arrow))) return !0;
						}return !1;
						...
						var t = this;return this.tsTryParseAndCatch(function () {
							var e = t.startNode();t.expectRelational("<");var s = t.tsParseDelimitedList("TypeParametersOrArguments", t.tsParseType.bind(t));return t.expectRelational(">"), e.params = s, t.finishNode(e, "TypeParameterInstantiation"), t.expect(C.parenL), e;
						});
						...
						t.id = this.parseIdentifier(), t.typeParameters = this.tsTryParseTypeParameters(), this.eat(C._extends) && (t.extends = this.tsParseHeritageClause());var e = this.startNode();return e.body = this.tsParseObjectTypeMembers(), t.body = this.finishNode(e, "TSInterfaceBody"), this.finishNode(t, "TSInterfaceDeclaration");
					}, e.prototype.tsParseTypeAliasDeclaration = function (t) {
						return t.id = this.parseIdentifier(), t.typeParameters = this.tsTryParseTypeParameters(), this.expect(C.eq), t.typeAnnotation = this.tsParseType(), this.semicolon(), this.finishNode(t, "TSTypeAliasDeclaration");
					}, e.prototype.tsParseEnumMember = function () {
						var t = this.startNode();return t.id = this.match(C.string) ? this.parseLiteral(this.state.value, "StringLiteral") : this.parseIdentifier(!0), this.eat(C.eq) && (t.initializer = this.parseMaybeAssign()), this.finishNode(t, "TSEnumMember");
					}, e.prototype.tsParseEnumDeclaration = function (t, e) {
						return e && (t.const = !0), t.id = this.parseIdentifier(), this.expect(C.braceL), t.members = this.tsParseDelimitedList("EnumMembers", this.tsParseEnumMember.bind(this)), this.expect(C.braceR), this.finishNode(t, "TSEnumDeclaration");
					}, e.prototype.tsParseModuleBlock = function () {
						var t = this.startNode();return this.expect(C.braceL), this.parseBlockOrModuleBlockBody(t.body = [], void 0, !0, C.braceR), this.finishNode(t, "TSModuleBlock");
						...
						return this.isContextual("require") && this.lookahead().type === C.parenL;
					}, e.prototype.tsParseModuleReference = function () {
						return this.tsIsExternalModuleReference() ? this.tsParseExternalModuleReference() : this.tsParseEntityName(!1);
					}, e.prototype.tsParseExternalModuleReference = function () {
						var t = this.startNode();if (this.expectContextual("require"), this.expect(C.parenL), !this.match(C.string)) throw this.unexpected();return t.expression = this.parseLiteral(this.state.value, "StringLiteral"), this.expect(C.parenR), this.finishNode(t, "TSExternalModuleReference");
						...
						switch (this.state.type) {case C._function:
								return this.next(), this.parseFunction(t, !0);case C._class:
								return this.parseClass(t, !0, !1);case C._const:
								if (this.match(C._const) && this.lookaheadIsContextual("enum")) return this.expect(C._const), this.expectContextual("enum"), this.tsParseEnumDeclaration(t, !0);case C._var:case C._let:
								return this.parseVarStatement(t, this.state.type);case C.name:
						...
						var e = this.lookahead();return e.type === C.name && e.value === t;
						...
						switch (e) {case "abstract":
								...
								if (s || this.match(C.name)) return s && this.next(), this.tsParseEnumDeclaration(t, !1);break;case "interface":
								if (s || this.match(C.name)) return s && this.next(), this.tsParseInterfaceDeclaration(t);break;case "module":
								if (s && this.next(), this.match(C.string)) return this.tsParseAmbientExternalModuleDeclaration(t);if (s || this.match(C.name)) return this.tsParseModuleOrNamespaceDeclaration(t);break;case "namespace":
								if (s || this.match(C.name)) return s && this.next(), this.tsParseModuleOrNamespaceDeclaration(t);break;case "type":
								if (s || this.match(C.name)) return s && this.next(), this.tsParseTypeAliasDeclaration(t);}
						...
						!i && this.match(C.colon) && (e.returnType = this.tsParseTypeOrTypePredicateAnnotation(C.colon));var r = "FunctionDeclaration" === s ? "TSDeclareFunction" : "ClassMethod" === s ? "TSDeclareMethod" : void 0;r && !this.match(C.braceL) && this.isLineTerminator() ? this.finishNode(e, r) : t.prototype.parseFunctionBodyAndFinish.call(this, e, s, i);
						...
						if (this.eat(C.bang)) {
							...
							}var h = this.startNodeAt(s, i);h.callee = e;var p = this.tsTryParseTypeArgumentsInExpression();if (p) return h.arguments = this.parseCallExpressionArguments(C.parenR, !1), h.typeParameters = p, this.finishCallExpression(h);
						...
						var s = this;if (this.isRelational("<")) {
							var i = this.tsTryParseAndCatch(function () {
								var t = s.tsParseTypeArguments();return s.match(C.parenL) || s.unexpected(), t;
							});i && (e.typeParameters = i);
						}t.prototype.parseNewArguments.call(this, e);
					}, e.prototype.parseExprOp = function (e, s, i, r, a) {
						if (y(C._in.binop) > r && !this.hasPrecedingLineBreak() && this.eatContextual("as")) {
						...
						return this.match(C.name) && this.lookahead().type === C.eq ? this.tsParseImportEqualsDeclaration(e) : t.prototype.parseImport.call(this, e);
					}, e.prototype.parseExport = function (e) {
						if (this.match(C._import)) return this.expect(C._import), this.tsParseImportEqualsDeclaration(e, !0);if (this.eat(C.eq)) {
						...
						if (this.state.type === C._const) {
							var i = this.lookahead();if (i.type === C.name && "enum" === i.value) {
								var r = this.startNode();return this.expect(C._const), this.expectContextual("enum"), this.tsParseEnumDeclaration(r, !0);
							}
						...
						if (this.match(C.name)) switch (this.state.value) {case "abstract":case "declare":case "enum":case "interface":case "module":case "namespace":case "type":
								return !0;}return t.prototype.shouldParseExportDeclaration.call(this);
					}, e.prototype.parseConditional = function (e, s, i, r, a) {
						if (!a || !this.match(C.question)) return t.prototype.parseConditional.call(this, e, s, i, r, a);var n = this.state.clone();try {
							return t.prototype.parseConditional.call(this, e, s, i, r);
						} catch (t) {
						...
						var s = this.eatContextual("declare"),
						    i = void 0;return this.match(C.name) && (i = this.tsTryParseExportDeclaration()), i || (i = t.prototype.parseExportDeclaration.call(this, e)), i && s && (i.declare = !0), i;
					}, e.prototype.parseClassId = function (e, s, i) {
						...
						return this.match(C.colon) && (e.returnType = this.tsParseTypeAnnotation()), t.prototype.parseAsyncArrowFromCallExpression.call(this, e, s);
						...
						if (this.match(C.colon)) {
							var s = this.state.clone();try {
								var i = this.tsParseTypeOrTypePredicateAnnotation(C.colon);this.canInsertSemicolon() && this.unexpected(), this.match(C.arrow) || this.unexpected(), e.returnType = i;
							} catch (t) {
								if (!(t instanceof SyntaxError)) throw t;this.state = s;
							}
						}return t.prototype.parseArrow.call(this, e);
						...
						switch (this.state.type) {case C._this:
			...
			};et.estree = it, et.flow = nt, et.jsx = ct, et.typescript = lt;var ut = {};e.parse = v, e.parseExpression = P, e.tokTypes = C;
