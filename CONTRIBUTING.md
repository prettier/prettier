...
- **`parsers`**: A list of parser names. The tests verify that the parsers in this list produce the same output. If the list includes `tests/format/typescript`, then `babel-ts` is included implicitly. If the list includes `tests/format/flow` or `tests/format/flow-repo`, then `babel-flow` is included implicitly. If the list includes `babel`, and the current directory is inside `tests/format/js`, then `acorn`, `espree`, and `meriyah` are included implicitly.
