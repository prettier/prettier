...
You can run `yarn build && FULL_TEST=1 jest` for a more robust test run, which includes the following additional checks:
...
- Additional typing-related checks that require types generated by `yarn build` (and so only run with `FULL_TEST=1` set)
