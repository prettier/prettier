...
options like `printWidth`). A _printer_'s job is to take
...
Examples of this are printing CSS-in-JS or fenced code blocks in Markdown.
Its signature is:
...
is passed an additional `textToDoc` function, which can be used to render
...
algorithm. Once the comments are attached to the AST, Prettier will automatically call
...
or return `false` and let Prettier attach the comment.
...
The `util.addTrailingComment`/`addLeadingComment`/`addDanglingComment`
