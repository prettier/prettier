...
turns into a string (based on options like `textWidth`). A _printer_'s job is to
take the AST generated by `parsers[<parser name>].parse` and return a Doc. A Doc
...
The printing processes works as follows:
...
inside another. Examples of this are printing CSS-in-JS or Markdown in code
blocks. Its signature is:
...
additional `textToDoc` function, which can be used render a doc using a
...
function embed(path, print, textToDoc, options) {
  ...
  if (node.type === "javascript") {
    return textToDoc(node.javascriptText, { ...options, parse: "babel" });
  }
  return false;
}
```
...
Once attached to the AST, Prettier will automatically call the
...
attach a comment to a node and return `true`, or return a `falsy` value and let
Prettier attach the comment.
...
_Manually attaching a comment_

Nodes with comments are expected to have a `comments` property containing an
array of comments. The
`util.addTrailingComment`/`addLeadingComment`/`addRemainingComment` functions
...
This example uses `util.addTrailingComment`, which automatically sets
