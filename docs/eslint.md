...
So you'll probably want to disable all of ESLint's formatting rules (while keeping around non-formatting rules that Prettier doesn't care about).  The easiest way to do this is to use [eslint-config-prettier](https://github.com/prettier/eslint-config-prettier). It's a one liner that can be added on-top of any existing ESLint configuration.
