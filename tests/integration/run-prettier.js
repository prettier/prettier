...
function runCliWorker(dir, args, options) {
  ...
  return new Promise((resolve, reject) => {
    worker.on("exit", async (code) => {
      result.status = code;
      result.stdout = trimFinalNewLine(await streamToString(worker.stdout));
      result.stderr = trimFinalNewLine(await streamToString(worker.stderr));
      resolve(result);
    });
